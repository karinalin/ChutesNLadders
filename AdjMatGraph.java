...
public class AdjMatGraph<T> implements Graph<T>, Iterable<T> {
 ...
 protected static final int DEFAULT_CAPACITY = 1; // Small so that we can test expand
 protected static final boolean VERBOSE = false;  // print while reading TGF?
 ...
 public static void loadTGF(String tgf_file_name, AdjMatGraph<String> g) throws FileNotFoundException {
  ...
  try {
   ...
   public AdjMatGraph(String inFileName){
    ...
    n = 0;
    this.arcs = new boolean[1][1];
    this.vertices = (T[])(new Object[1]);
    
    try{
      // set up Scanner to input file: this line could generate an exception
      Scanner scan = new Scanner(new File(inFileName));
      
      boolean passedHash = false;
      ...
      while (scan.hasNextLine()) {
        ...
        if (line.equals("#")) {
          ...
          int index = (Integer.parseInt(lineEntries[0]) - 1);
          
          T vertex = (T)(lineEntries[1]);
          
          addVertex(vertex);
        }
        
        // treat the lines after the pound key: ignoring the line
        // with the pound key and the final empty line
        else if (passedHash && !line.equals("#") && !line.equals("")) {
          
          //creating the adjacency matrix
          String[] connection = line.split(" ");
          Integer oriIndex = (Integer.parseInt(connection[0])-1);
          Integer destIndex = (Integer.parseInt(connection[1])-1);
          addArc(vertices[oriIndex], vertices[destIndex]);
        } 
      }
